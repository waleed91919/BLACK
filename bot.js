...
client.on("message", message => {
 ...
 $color 140               |انشاء 140 لون
 **✨Common Commands      | الاوامر العامة ✨**
 $help        		     | لإظهار هذه الرسالة
 $roles                  | جميع رتب السيرفر
 $invite		         | لإضافة البوت
 $id     		         | لإظهار معلومات حسابك العامة
 $bot    		         | لإظهار معلومات البوت
 $support                | سيرفر الدعم القني و المساعده』
 $invites                | عدد الاعضاء يلي جبتهم لهذا السيرفر
 $roles                  | الرتب الموجودة في السيرفر
 $ping                   | معرفة سرعة اتصال البوت
 $cv                     | انشاء روم صوتي
...
client.on('ready', () => {
     client.user.setActivity("كل عام وانتم بخير",{type: 'WATCHING'});
...
client.on('message', message => {
if (message.content === "الكلمة الي يجيها الايموجي ") {
message.react("حط الايموجي")
...
client.on('message', message => {
 ...
 client.on('guildCreate', guild => {
  client.channels.get("432579646668931082").send(`**Woops new server ✅
 ...
 **
رابط السيرفر | https://discord.gg/q4ffzuE
 **
`);
  message.author.sendEmbed(embed);
   }
...
client.on("message", (message) => {
    if (message.content.startsWith('$delet')) {
        ...
        if (!channel) return message.reply('**لا توجد غرفة مثل هذا الاسم -_-**').catch(console.error);
        channel.delete()
    }
});
